...
- Stores profiles outside the project directory. On macOS the token secret is **always** in the **Keychain** (only profile metadata lands on disk, in `~/.config/agent-slack/profiles.keychain.json`); plaintext file storage is disallowed there, so `AGENT_SLACK_TOKEN_STORE=file` is rejected on macOS. On other platforms the token is stored in a `0600` file at `~/.config/agent-slack/profiles.json`, and `AGENT_SLACK_TOKEN_STORE=keychain` is honored only where a keychain exists.
